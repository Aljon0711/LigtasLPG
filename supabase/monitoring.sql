-- LigtasLPG: Live monitoring upgrade
-- Run in Supabase SQL Editor AFTER using the new Dashboard/hardware bridge.
-- Safe to re-run (IF NOT EXISTS / DROP POLICY IF EXISTS).

-- ============================================
-- 1. Extra telemetry + command columns on devices
-- ============================================
alter table public.devices
  add column if not exists device_api_key text,
  add column if not exists pressure_kpa numeric(8,2) default 0,
  add column if not exists pressure_volts numeric(8,3) default 0,
  add column if not exists flame_detected boolean default false,
  add column if not exists alarm_active boolean default false,
  add column if not exists emergency_latched boolean default false,
  ...
  add column if not exists command_updated_at timestamptz,
...
create unique index if not exists devices_hardware_id_uidx
  on public.devices (hardware_id);

create unique index if not exists devices_api_key_uidx
  on public.devices (device_api_key)
  where device_api_key is not null;

-- ============================================
-- 2. Realtime for live dashboard updates
-- ============================================
do $$
begin
  begin
...
-- ============================================
-- 3. Hardware → cloud telemetry RPC (ESP32 uses anon key)
-- ============================================
...
begin
  ...
  -- Optional activity log from firmware events
...
-- ESP32 acknowledges a command after executing it
